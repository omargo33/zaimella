CREATE OR REPLACE PACKAGE BODY "DATA"."PK_BT01_ARTICULOS_JDE" AS
 ...
 -- Buscar el siguiente id de movimiento de inventario
            SELECT
                NVL(MAX(ID_MOVIMIENTOS_INVENTARIO), 0) + 1 INTO V_IDMOVIMIENTOSINVENTARIO
            FROM
 ...
 --Buscar el siguiente id de detalle de movimiento de inventario
            SELECT
                NVL(MAX(ID_DETALLE_MOVTOS_ENTRADA), 0) + 1 INTO V_IDDETALLEMOVTOSENTRADA
            FROM
 ...
 -- Insertar detalle de movimiento de inventario
        BEGIN
            ...
            SELECT
                NVL(MAX(ID_DETALLE_MOVTOS_INVENT), 0) + 1 INTO V_IDDETALLEMOVTOSINVENT
            FROM
 ...
 -- crear variables para la consulta sql SELECT ID, USUARIO, USUARIOFECHA, VALORTEXTO, VALORNUMERO, MODO, CODIGOITEM, NOMBREITEM, DESCRIPCIONITEM, UNIDADMEDIDA, CLASIFICACION, CLASIFICACION2, ALMACEN, COSTO FROM T_BT01_ARTICULO;
        V_MODO                    VARCHAR2(8);
        ...
        V_COSTO                   NUMBER;
        V_ID_HERRAMIENTA_REPUESTO NUMBER;
    BEGIN
        BEGIN
            <<BUSCAR_INFO_WS>>
            SELECT
 ...
 -- procesa repuesto
            IF V_MODO = 'R' THEN
                SP_PROCESA_ARTICULO_REP( V_CODIGOITEM, V_NOMBREITEM, V_DESCRIPCIONITEM, V_UNIDADMEDIDA, V_CLASIFICACION, V_CLASIFICACION2, V_ALMACEN, V_COSTO, P_MENSAJE => P_MENSAJE, P_ID_REPUESTO => V_ID_HERRAMIENTA_REPUESTO );
                IF (P_MENSAJE IS NOT NULL) THEN
                    RETURN;
                END IF;
            ELSE
 ...
 -- procesa herramienta
                IF V_MODO = 'H' THEN
                    SP_PROCESA_ARTICULO_HER( V_CODIGOITEM, V_NOMBREITEM, V_DESCRIPCIONITEM, V_UNIDADMEDIDA, V_CLASIFICACION, V_CLASIFICACION2, V_ALMACEN, V_COSTO, P_MENSAJE => P_MENSAJE, P_ID_HERRAMIENTA => V_ID_HERRAMIENTA_REPUESTO );
                    IF (P_MENSAJE IS NOT NULL) THEN
                        RETURN;
                    END IF;
                ELSE
 ...
 --
 -- Actualiza el ID de la herramienta o repuesto
        BEGIN
            <<ACTUALIZAR_INFO_WS>>
            UPDATE DATA.T_BT01_ARTICULO
            SET
                VALORNUMERO = V_ID_HERRAMIENTA_REPUESTO
            WHERE
                ID = P_ID;
        EXCEPTION
            WHEN OTHERS THEN
                P_MENSAJE := 'No se pudo actualizar el articulo con el ID: '
                             || P_ID;
                RAISE_APPLICATION_ERROR(-20001, P_MENSAJE);
                RETURN;
        END ACTUALIZAR_INFO_WS;
    END SP_PROCESARARTICULO; --
 -- procesar alamacen de los articulos T_BT01_ARTICULO desde el ID
    PROCEDURE SP_PROCESARALMACEN(
        P_ID IN NUMBER,
        P_MENSAJE OUT VARCHAR2
    ) AS
 -- secuencia para el ID de los almacenes
        V_ID_HERRAMIENTA_REPUESTO NUMBER;
 -- variables
        V_MODO                    VARCHAR2(8);
        V_CODIGOITEM              VARCHAR2(32);
 ...
 -- procesa repuesto
            IF V_MODO = 'R' THEN
                SP_PROCESA_ALMACEN_REP( V_CODIGOITEM, V_ALMACEN, V_COSTO, P_MENSAJE => P_MENSAJE, P_ID_REPUESTO => V_ID_HERRAMIENTA_REPUESTO );
                IF (P_MENSAJE IS NOT NULL) THEN
                    RETURN;
                END IF;
            END IF;
 ...
 -- procesa herramienta
            IF V_MODO = 'H' THEN
                SP_PROCESA_ALMACEN_HER( V_CODIGOITEM, V_ALMACEN, V_COSTO, P_MENSAJE => P_MENSAJE, P_ID_HERRAMIENTA => V_ID_HERRAMIENTA_REPUESTO );
                IF (P_MENSAJE IS NOT NULL) THEN
                    RETURN;
                END IF;
            END IF;
 ...
 --
 -- buscar si hay kardex
            BEGIN
                <<BUSCAR_KARDEX_REPUESTO>>
                SELECT
                    COUNT(ID_KARDEX_RPTOS_HERR) INTO V_ID_KARDEX_RPTOS_HERR
                FROM
                    MPZML_REP.KARDEX_RPTOS_HERR
                WHERE
                    ID_CAT_RPTOS_HERR = V_ID_CAT_REPUESTOS
                    AND ID_CAT_ALMACENES = V_ID_CAT_ALMACENES;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    V_ID_KARDEX_RPTOS_HERR:=NULL;
            END BUSCAR_KARDEX_REPUESTO;
 --
 -- insertar kardex de no haberlo
            IF V_ID_KARDEX_RPTOS_HERR = 0 THEN
 --
 -- buscar el maximo como indice
                BEGIN
                    <<MAXIMO_KARDEX>>
                    SELECT
                        NVL(MAX(ID_KARDEX_RPTOS_HERR), 0) + 1 INTO V_ID_KARDEX_RPTOS_HERR
                    FROM
                        MPZML_REP.KARDEX_RPTOS_HERR;
                EXCEPTION
                    WHEN OTHERS THEN
 ...
 -- Obtiene el indice de repuestos para elementos nuevos
            BEGIN
                ...
                SELECT
                    NVL(MAX(ID_CAT_REPUESTOS), 0)+1 INTO V_ID_CAT_REPUESTOS
                FROM
                    MPZML_REP.CAT_REPUESTOS;
            EXCEPTION
                WHEN OTHERS THEN
 ...
 -- Insertar Kardex repuestos herramientas
            ...
            END INSERTAR_KARDEX;
        END IF;
        P_ID_REPUESTO := V_ID_CAT_REPUESTOS;
 ...
 -- buscar si hay kardex
            BEGIN
                <<BUSCAR_KARDEX_HERRAMIENTA>>
                SELECT
                    COUNT(ID_KARDEX_RPTOS_HERR) INTO V_ID_KARDEX_RPTOS_HERR
                FROM
                    MPZML_HER.KARDEX_RPTOS_HERR
                WHERE
                    ID_CAT_RPTOS_HERR = V_ID_CAT_HERRAMIENTAS
                    AND ID_CAT_ALMACENES = V_ID_CAT_ALMACENES;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    V_ID_KARDEX_RPTOS_HERR:=NULL;
            END BUSCAR_KARDEX_HERRAMIENTA;
 --
 -- insertar kardex de no haberlo
            IF V_ID_KARDEX_RPTOS_HERR = 0 THEN
 --
 ...
 -- Obtiene el indice de herramientas para elementos nuevos
            BEGIN
                ...
                SELECT
                    NVL(MAX(ID_CAT_HERRAMIENTAS), 0) + 1 INTO V_ID_CAT_HERRAMIENTAS
                FROM
                    MPZML_HER.CAT_HERRAMIENTAS;
            EXCEPTION
                WHEN OTHERS THEN
 ...
 -- Insertar Kardex repuestos herramientas
            ...
            END INSERTAR_KARDEX;
        END IF;
        P_ID_HERRAMIENTA := V_ID_CAT_HERRAMIENTAS;
 ...
 -- indice de almacen
    ...
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                P_MENSAJE := 'No se pudo encontrar la herramienta para: '
                             || P_CODIGOITEM;
                RAISE_APPLICATION_ERROR(-20001, P_MENSAJE);
                RETURN;
        END BUSCAR_HERRAMIENTA;
 ...
 -- insertar kardex
            ...
            END INSERTAR_KARDEX;
        ELSE
 -- caso contrario enivar mensaje mostrando el error.
            P_MENSAJE := 'El almacen: '
                                                                              || P_ALMACEN
                                                                              || ' ya existe para la herramienta: '
                                                                              || P_CODIGOITEM;
            RAISE_APPLICATION_ERROR(-20001, P_MENSAJE);
            RETURN;
        END IF;
    END SP_PROCESA_ALMACEN_HER; --
 ...
 -- secuencia de repuestos
        V_ID_KARDEX_RPTOS_REP NUMBER;
 -- indice de almacen
        V_ID_CAT_REPUESTOS    NUMBER;
        V_ID_CAT_ALMACENES    NUMBER;
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS THEN
                P_MENSAJE := 'No se pudo encontrar el repuesto para: '
                             || P_CODIGOITEM;
                RAISE_APPLICATION_ERROR(-20001, P_MENSAJE);
                RETURN;
        END BUSCAR_REPUESTO;
 ...
 -- buscar si hay kardex
        BEGIN
            <<BUSCAR_KARDEX_REPUESTO>>
            SELECT
                COUNT(ID_KARDEX_RPTOS_HERR) INTO V_ID_KARDEX_RPTOS_REP
            FROM
                MPZML_REP.KARDEX_RPTOS_HERR
            WHERE
                ID_CAT_RPTOS_HERR = V_ID_CAT_REPUESTOS
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                V_ID_KARDEX_RPTOS_REP:=NULL;
        END BUSCAR_KARDEX_REPUESTO;
 --
 -- insertar kardex de no haberlo
        IF V_ID_KARDEX_RPTOS_REP = 0 THEN
 --
 -- buscar el maximo como indice
            BEGIN
                <<MAXIMO_KARDEX>>
                SELECT
                    NVL(MAX(ID_KARDEX_RPTOS_HERR), 0) + 1 INTO V_ID_KARDEX_RPTOS_REP
                FROM
                    MPZML_REP.KARDEX_RPTOS_HERR;
            EXCEPTION
                WHEN OTHERS THEN
